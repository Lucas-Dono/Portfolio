  ...
  useEffect(() => {
    ...
    const cargarPrecios = async () => {
      ...
      try {
        ...
        try {
          console.log('Intentando obtener timestamp desde:', getApiUrl('/servicios/last-updated'));
          ...
          if (timestampResponse.ok) { // Verificar que la respuesta sea exitosa
            ...
            const alternativeUrl = `${window.location.protocol}//${window.location.hostname}:${window.location.port}/api/servicios/last-updated`;
            console.log('Intentando URL alternativa:', alternativeUrl);
            const altResponse = await fetch(alternativeUrl, { credentials: 'include' });
            if (altResponse.ok) {
              const altData = await altResponse.json();
              if (altData && altData.lastUpdated) {
                lastUpdated = altData.lastUpdated;
                console.log('✅ Timestamp obtenido con URL alternativa:', lastUpdated);
              }
            }
          }
